...
class Common extends Controller
{
	...
	public function __construct(){
        ...
        if(Session::get('login_id','fei_chart')){
        	$this->assign('user_id',Session::get('login_id','fei_chart'));
            $this->assign('user_email',Session::get('login_email','fei_chart'));
            $this->assign('user_name',Session::get('login_name','fei_chart'));
        	$this->assign('user_nikename',Session::get('login_nikename','fei_chart'));
            Log::log('登录日志 --- 开始');
            Log::log('ID: '.Session::get('login_id','fei_chart').' 访问了 '.$request->controller().'/'.$request->action());
            Log::log('登录日志 --- 结束');
        }
